...
Gvrp_model::Gvrp_model(const Gvrp_instance& instance, unsigned int time_limit) : Cplex_model(instance, time_limit), lambda(calculateGvrpInstanceLambda(instance)), psi(calculateGvrpInstancePsi(instance)) {
  if (instance.distances_enum != METRIC)
    throw string("Error: The compact model requires a G-VRP instance with metric distances");
  alpha = min(lambda, psi);
  //gvrp afs tree
